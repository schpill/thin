    ...
    class Data
    {
        ...
        public static function getAll($type)
        {
            ...
            if (ake($type, static::$_all)) {
                ...
                $objects    = glob(STORAGE_PATH . DS . 'data' . DS . $dir . DS . 'read' . DS . '*.data');
            }
            if (!count($objects)) {
                static::event($type);
            }
        ...
        private static function _clean($directory, $type)
        {
            ...
            $firstFiles     = glob($firstDirectory . DS . '*.data');
        ...
        public static function indexCreate($indexField, $indexInfo, $object)
        {
            ...
            if ('fulltext' == $typeIndex) {
                ...
                $continue = true;
                if (is_dir($dirIndex)) {
                    $objectsDir = glob($dirIndex . DS . '*.data');
                        ...
                        if ('fulltext' == $typeIndex) {
                            ...
                            if (count($words)) {
                                foreach ($words as $word) {
                                    ...
                                    $objects        = glob($indexWordDir . DS . '*.data');
                                    if (count($objects)) {
