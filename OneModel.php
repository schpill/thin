<?php
    class OneWrapper extends One
    {
         ...
         * One wrapper as its first argument and return an instance
         * of the One wrapper. Any arguments passed to this method
         * after the name of the filter will be passed to the called
         * filter function as arguments after the One class.
         *
         * @return OneWrapper
         */
         ...
         * created class is OneWrapper, not One
         *
         ...
         * it with the supplied One instance.
         *
         * @param  One $One
         * @return bool|Model
         */
        protected function _createModelInstance($One)
        {
            if ($One === false) {
                return false;
            }
    ...
    class OneModel
    {
         ...
         *
         * @example OneModel::$auto_prefix_models = 'MyProject_MyModels_'; //PEAR
         * @example OneModel::$auto_prefix_models = '\MyProject\MyModels\'; //Namespaces
         *
        ...
        /**
         * The One instance used by this model
         ...
         * @return string
         */
        protected static function _getStaticProperty($class_name, $property, $default = null)
        {
         ...
         * the _classNameToTableName method method.
         *
         * If public static property $_table_use_short_name == true
         * then $class_name passed to _classNameToTableName is
         * stripped of namespace infOneation.
         *
         * @param  string $class_name
         * @return string
         */
        protected static function _getTableName($class_name)
        {
            $specified_table_name = self::_getStaticProperty($class_name, '_table');
            $use_short_class_name = self::_getStaticProperty($class_name, '_table_use_short_name');
            ...
            if (is_null($specified_table_name)) {
                return self::_classNameToTableName($class_name);
            }
        ...
        /**
         * Convert a namespace to the standard PEAR underscore fOneat.
         *
         ...
         * @return string
         */
        protected static function _classNameToTableName($class_name)
        {
            return Thin\Inflector::lower(preg_replace(
                ['/\\\\/', '/(?<=[a-z])([A-Z])/', '/__/'],
                ['_', '_$1', '_'],
                ltrim($class_name, '\\')
            ));
         ...
         * @return string|null
         */
        protected static function _getIdColumnName($class_name)
        {
            return self::_getStaticProperty($class_name, '_id_column', self::DEFAULT_ID_COLUMN);
        }
         ...
         * @return string
         */
        protected static function _buildForeignKeyName($specified_foreign_key_name, $table_name)
        {
         ...
         * should exist). This method actually returns a wrapped One object
         * which allows a database query to be built. The wrapped One object is
        ...
        public static function factory($class_name, $connection_name = null)
        {
            ...
            $table_name = self::_getTableName($class_name);

            if ($connection_name == null) {
                $connection_name = self::_getStaticProperty(
                   $class_name,
                   '_connection_name',
                   OneWrapper::DEFAULT_CONNECTION
                );
            }

            $wrapper = OneWrapper::for_table($table_name, $connection_name);
            $wrapper->set_class_name($class_name);
            $wrapper->use_id_column(self::_getIdColumnName($class_name));
         ...
         * @return OneWrapper
         */
        protected function _hasOneOrMany($associated_class_name, $foreign_key_name = null, $foreign_key_name_in_current_models_table = null, $connection_name = null)
        {
            $base_table_name = self::_getTableName(get_class($this));
            $foreign_key_name = self::_buildForeignKeyName($foreign_key_name, $base_table_name);
        ...
        {
            return $this->_hasOneOrMany($associated_class_name, $foreign_key_name, $foreign_key_name_in_current_models_table, $connection_name);
        }
        ...
        {
            $associated_table_name = self::_getTableName(self::$auto_prefix_models . $associated_class_name);
            $foreign_key_name = self::_buildForeignKeyName($foreign_key_name, $associated_table_name);
        ...
        protected function has_many_through($associated_class_name, $join_class_name = null, $key_to_base_table = null, $key_to_associated_table = null,  $key_in_base_table = null, $key_in_associated_table = null, $connection_name = null)
        {
            ...
            // fOneed by concatenating the names of the base class
            ...
            $base_table_name = self::_getTableName($base_class_name);
            $associated_table_name = self::_getTableName(self::$auto_prefix_models . $associated_class_name);
            $join_table_name = self::_getTableName(self::$auto_prefix_models . $join_class_name);

            // Get ID column names
            $base_table_id_column = (is_null($key_in_base_table)) ?
                self::_getIdColumnName($base_class_name) :
                $key_in_base_table;
            $associated_table_id_column = (is_null($key_in_associated_table)) ?
                self::_getIdColumnName(self::$auto_prefix_models . $associated_class_name) :
                $key_in_associated_table;

            // Get the column names for each side of the join table
            $key_to_base_table = self::_buildForeignKeyName($key_to_base_table, $base_table_name);
            $key_to_associated_table = self::_buildForeignKeyName($key_to_associated_table, $associated_table_name);
        ...
        /**
         * Set the wrapped One instance associated with this Model instance.
         *
         * @param  One $One
         * @return void
         */
        public function set_one($One)
        {
            $this->One = $One;
        }
        ...
        {
            $this->One->set_expr($property, $value);
        ...
        /**
         * Wrapper for One's as_array method.
         *
         * @return Array
         */
        public function as_array()
        {
            ...
            return call_user_func_array([$this->One, 'as_array'], $args);
        }
        ...
        /**
         * Calls static methods directly on the OneWrapper
         *
         * @param  string $method
         ...
         * @throws OneModelMethodMissingException
         * @return bool|OneWrapper
         */
        public function __call($name, $arguments)
        {
            $method = Thin\Inflector::lower(preg_replace('/([a-z])([A-Z])/', '$1_$2', $name));

            if (method_exists($this, $method)) {
                return call_user_func_array([$this, $method], $arguments);
            } else {
                throw new OneModelMethodMissingException("Method $name() does not exist in class " . get_class($this));
            }
        }
    }

    class OneModelMethodMissingException extends Thin\Exception {}
