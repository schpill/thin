    ...
    class smb
    {
                ...
                switch ($tag) {
                    ...
                    case 'files':
                        ...
                        $i = ($name <> '.' && $name <> '..')
                            ...
                            : array();
                        break;
                    case 'error':   error_log($regs[1], E_USER_ERROR);
                }
        ...
        function unlink ($url)
        {
            ...
            if ($pu['type'] <> 'path') error_log('unlink(): error in URL', E_USER_ERROR);
        ...
        function rename ($url_from, $url_to)
        {
            ...
            if ($from['host'] <> $to['host'] ||
                ...
                error_log('rename(): FROM & TO must be in same server-share-user-pass-domain', E_USER_ERROR);
            }
            if ($from['type'] <> 'path' || $to['type'] <> 'path') {
                error_log('rename(): error in URL', E_USER_ERROR);
            }
        ...
        {
            $pu = smb::parse_url($url);
            if ($pu['type'] <> 'path') error_log('mkdir(): error in URL', E_USER_ERROR);
        ...
        {
            $pu = smb::parse_url($url);
            if ($pu['type'] <> 'path') error_log('rmdir(): error in URL', E_USER_ERROR);
            smb::clearstatcache ($url);
