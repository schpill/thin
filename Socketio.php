    ...
    class Socketio
    {
        ...
        public function on($event, $callback)
        {
            if (!is_callable($callback)) {
                throw new Exception('Socketio::on() type callback must be callable.');
            }
        ...
        public function send($type, $id = null, $endpoint = null, $message = null)
        {
            if (!is_int($type) || $type > 8) {
                throw new Exception('Socketio::send() type parameter must be an integer strictly inferior to 9.');
            }
        ...
        private function stdout($type, $message)
        {
            ...
            if (!Arrays::exists($type, $typeMap)) {
                throw new Exception('Socketio::stdout $type parameter must be debug, info, error or success. Got '.$type);
            }
