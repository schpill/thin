<?php
	...
	class Tool
	{
		...
		{
			return Arrays::exists($name, static::$registry);
		}
		 ...
		 *		Tool::instance('mailer', new Mailer);
		 * </code>
		 *
		 * @param  string  $name
		...
		public static function resolve($type, $parameters = array())
		{
			...
			if ( ! isset(static::$registry[$type])) {
				...
				$concrete = arrayGet(static::$registry[$type], 'resolver', $type);
			}
			...
			fire('thin.resolving', array($type, $object));
