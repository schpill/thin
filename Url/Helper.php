    ...
    class Helper
    {
        ...
        public static function segmentizeUrl($url)
        {
            ...
            foreach ($segments as $segment) {
                if (strlen($segment)) $result[] = $segment;
            }
        ...
        public static function rebuildUrl(array $urlArray)
        {
            ...
            foreach ($urlArray as $segment) {
                if (strlen($segment)) $url .= '/' . trim($segment);
            }
        ...
        public static function parseValues($object, array $columns, $string)
        {
            ...
            foreach ($columns as $column) {
                if (!isset($object->{$column}) || is_array($object->{$column})) continue;
        ...
        public static function segmentIsOptional($segment)
        {
            ...
            if ($optMarkerPos === false) return false;

            $regexMarkerPos = mb_strpos($name, '|');

            if ($regexMarkerPos === false) return true;

            if ($optMarkerPos !== false && $regexMarkerPos !== false) return $optMarkerPos < $regexMarkerPos;
        ...
        public static function getParameterName($segment)
        {
            ...
            if ($optMarkerPos !== false && $regexMarkerPos !== false) {
                if ($optMarkerPos < $regexMarkerPos) return mb_substr($name, 0, $optMarkerPos);
                else return mb_substr($name, 0, $regexMarkerPos);
            }

            if ($optMarkerPos !== false) return mb_substr($name, 0, $optMarkerPos);

            if ($regexMarkerPos !== false) return mb_substr($name, 0, $regexMarkerPos);
        ...
        public static function getSegmentRegExp($segment)
        {
            if (($pos = mb_strpos($segment, '|')) !== false) {
                ...
                if (!mb_strlen($regexp)) return false;
        ...
        public static function getSegmentDefaultValue($segment)
        {
            ...
            if ($regexMarkerPos !== false) $value = mb_substr($segment, $optMarkerPos + 1, $regexMarkerPos-$optMarkerPos - 1);
            else $value = mb_substr($segment, $optMarkerPos + 1);
