<?php
    ...
    class Collection extends Customize
    {
        private $ns;
        private $entity;
        private $table;
        ...
        public function __construct($ns, $entity)
        {
            $this->ns       = $ns;
            $this->entity   = $entity;
            $this->db       = 'collection_' . $ns . '_' . $entity;
            $this->table    = 'collection_' . $ns . '::' . $entity;
        ...
        public function populate($data = array())
        {
            if (count($data)) {
                foreach ($data as $row) {
                    if (!Arrays::is($row)) $row = array('value' => $row);
        ...
        {
            $key = 'begin::collectionDb_' . $this->ns . '_' . $this->entity;
        ...
        private function add($data)
        {
            ...
            if (!strlen($exists)) {
                ...
                $this->lastInsertId = $id = $this->driver()->incr($this->ns . '_' . $this->entity . '_collectionCount');
                $data['id'] = $id;
                $data['created_at'] = $data['updated_at'] = time();
                $add = $this->driver()->set($this->table . '::' . $id, json_encode($data));
                $this->all(true);
        ...
        private function edit($id, $data)
        {
            ...
            if (!strlen($exists)) {
                ...
                $edit = $this->driver()->set($this->table . '::' . $id, json_encode($data));
                $this->all(true);
                $this->driver()->set($key, $id);
                $this->makeIndexes($data, $id);
                return $this->find($id);
        ...
        {
            $row = $this->driver()->get($this->table . '::' . $id);
            if (strlen($row)) {
        ...
        {
            $dbt = container()->cbm('cma_table');
            $dbf = container()->cbm('cma_field');
            $dbs = container()->cbm('cma_structure');
        ...
        {
            $dbt = container()->cbm('cma_table');
            $rows = $this->driver()->keys('_collectionCount');
            $tables = array();
            if (count($rows)) {
                foreach ($rows as $row) {
                    $seg = repl('_collectionCount', '', $row);
                    $tab = explode('_', $seg);
                    $ns = Arrays::first($tab);
                    $index = strReplaceFirst($ns . "_", "", $seg);
                    if (!strstr($index, 'cma_')) {
                        $t = $dbt->where('name = ' . $index)->where('ns = ' . $ns)->first(true);
                        if (is_null($t)) {
                            $tableName                  = "collection_" . $ns . "::$index";
                            $tables[$index]['count']    = $this->driver()->get($row);
                            $data                       = $this->driver()->keys($tableName);
                            if (count($data)) {
                                $first = Arrays::first($data);
                                $first = json_decode($this->driver()->get($first), true);
        ...
        {
            $check = $this->driver()->get($this->ns . '_' . $this->entity . '_collectionCount');
            if (!strlen($check)) {
                $this->driver()->set($this->ns . '_' . $this->entity . '_collectionCount', 0);
                return $this;
            }
            return false;
        }
        ...
        {
            $this->emptyTable();
            $this->driver()->del($this->ns . '_' . $this->entity . '_collectionCount');
            return $this;
        }
        ...
        public function all($force = false)
        {
            ...
            : array();
            if (empty($cached)) {
                $rows = $this->driver()->keys($this->table . '::');
                $collection = array();
                foreach ($rows as $k => $row) {
