<?php
    ...
    class Curl
    {
        ...
        {
            if ( ! $this->is_enabled()) {
                throw new e('cURL Class - PHP was not built with cURL enabled. Rebuild PHP with --with-curl to use cURL.');
            }
        ...
        public function simple_ftp_get($url, $filePath, $username = '', $password = '')
        {
        ...
        {
            $this->options[CURLOPT_CUSTOMREQUEST] = Inflector::upper($method);
            return $this;
        }

        public function http_login($username = '', $password = '', $type = 'any')
        {
            $this->option(CURLOPT_HTTPAUTH, constant('CURLAUTH_' . Inflector::upper($type)));
        ...
        public function ssl($verifyPeer = true, $verify_host = 2, $path_to_cert = null)
        {
            if ($verifyPeer) {
        ...
        public function option($code, $value)
        {
            if (is_string($code) && !is_numeric($code)) {
                $code = constant('CURLOPT_' . Inflector::upper($code));
            }
