<?php
	...
	class Eventable implements ArrayAccess
	{
		/**
		...
		/**
		 * The Eventable's shared instances.
		 *
		 * @var array
		 */
		...
		/**
		 * Register a binding with the Eventable.
		 *
		 * @param  string               $abstract
		...
		public function bind($abstract, $concrete = null, $shared = false)
		{
			...
			// alias with the Eventable so that it can be used as a shortcut for it.
			...
			// is bound into this Eventable to the abstract type and we will just wrap
			...
			// If the abstract type was already bound in this Eventable, we will fire the
		...
		/**
		 * Register a shared binding in the Eventable.
		 *
		 * @param  string               $abstract
		 * @param  Closure|string|null  $concrete
		 * @return void
		...
		{
			return function($Eventable) use ($closure) {
				// We'll simply declare a static variable within the Closures and if it has
				// not been set we will execute the given Closures to resolve this value
				// and return it back to these consumers of the method as an instance.
				static $object;
				if (is_null($object)) {
					$object = $closure($Eventable);
				}
				return $object;
			};
		}
		...
		/**
		 * Bind a shared Closure into the Eventable.
		 *
		 * @param  string  $abstract
		 * @param  \Closure  $closure
		 * @return void
		...
		/**
		 * "Extend" an abstract type in the Eventable.
		 *
		 * @param  string   $abstract
		 * @param  Closure  $closure
		 * @return void
		...
		protected function getExtender($abstract, Closure $closure)
		{
			...
			// handed off to the "new" resolver, along with this Eventable instance.
			$resolver = $this->bindings[$abstract]['concrete'];

			return function($Eventable) use ($resolver, $closure) {
				return $closure($resolver($Eventable), $Eventable);
			};
		}
		...
		/**
		 * Resolve the given type from the Eventable.
		 *
		 * @param  string  $abstract
		...
		protected function getConcrete($abstract)
		{
			...
			// since the Eventable should be able to resolve concretes automatically.
			if ( ! isset($this->bindings[$abstract])) {
		...
		public function build($concrete, $parameters = array())
		{
			...
			// resolving any other types or dependencies out of these Eventables.
		...
		/**
		 * Resolve a class based dependency from the Eventable.
		 *
		 * @param  \ReflectionParameter  $parameter
		 * @return mixed
		 *
		...
		/**
		 * Clear all of the instances from the Eventable.
		 *
		 * @return void
		 */
		...
		public function offsetSet($key, $value)
		{
			...
			// Eventable's simplest functions are base modeled and built after.
