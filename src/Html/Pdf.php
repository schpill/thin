<?php
    ...
    class Pdf
    {
        ...
        protected function _createFile()
        {
            ...
            } while(File::exists($this->_filename));

            /**
             * create an empty file
             */
            File::put($this->_filename, $this->getHtml());
        ...
        protected function _getCommand()
        {
            ...
            foreach($this->getMargins() as $position => $margin) {
                $command .= (false === is_null($margin)) ? sprintf(' --margin-%s %s', $position, $margin) : '';
            }
        ...
        public function output($mode, $filename)
        {
            switch ($mode) {
                case static::MODE_DOWNLOAD:
                    ...
                    break;
                case static::MODE_STRING:
                    return $this->_render();
                    break;
                case static::MODE_EMBEDDED:
                    if (!headers_sent()) {
                        $result = $this->_render();
                ...
                case static::MODE_SAVE:
                    File::put($this->getPath() . $filename, $this->_render());
                    $filepath = $this->getFilePath();
                    if (!empty($filepath)) {
                        unlink($filepath);
                    }
