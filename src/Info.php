<?php
	...
	class Info
	{
		...
		function  error($type)
		{
			...
			if(Arrays::inArray(substr($type, 0, 1), array("a", "e", "i", "o", "u", "x")))
				$error .= "n";
		...
		function varIsResource($var)
		{
			...
			switch(get_resource_type($var)) {
				...
				case "mysql result":
					$tab = explode(" ", get_resource_type($var));
					$db = current($tab);
			...
			echo $this->closeTDRow() . "</table><hr class=thinDebugHr />\n";
		}
		...
		{
			if($db == "pgsql") $db = "pg";
			if($db == "sybase-db" || $db == "sybase-ct") $db = "sybase";
			...
			for($i = 0 ; $i < $numfields ; $i++) {
				...
				for($j = 0 ; $j < count($arrFields) ; $j++) {
					...
					if(function_exists($db_func)) {
						...
						if($j == 0) $field_name = $fheader;
						else $field_header .= $fheader;
					}
				}
			...
			echo "</table>";
			if($numrows > 0) call_user_func($db . "_data_seek", $var, 0);
		}
		...
		function varIsXmlResource($var)
		{
			...
			//read xml file
			if(true === $bFile) {
				while($data = repl("\n", "", fread($fp, 4096))) $this->xmlParse($xml_parser, $data, feof($fp));
			}
		...
		function xmlCharacterData($parser, $data)
		{
			...
			if(!empty($this->xmlCData[$count])) $this->xmlCData[$count] .= $data;
			else $this->xmlCData[$count] = $data;
		}
		...
		function xmlDefaultHandler($parser,$data)
		{
			...
			$data = repl(array("&lt;!--", "--&gt;"), "", htmlspecialchars($data));
			$count = $this->xmlCount - 1;
			if(!empty($this->xmlDData[$count])) $this->xmlDData[$count] .= $data;
			else $this->xmlDData[$count] = $data;
		}
		...
		function initJSandCSS()
		{
			echo '
				...
				<style type="text/css">
					...
					table.dBug_resourceC td { background-color:#fff; }
					...
					table.dBug_resource td { background-color:#fff; }
