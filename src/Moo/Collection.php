	...
	class Collection implements ArrayAccess, Arrayable, Countable, IteratorAggregate, Jsonable, JsonSerializable
	{
		...
		public function min($field)
		{
			if (!$this->useAsCallable($field)) {
				return $this->min($this->valueRetriever($field));
			}
			...
			foreach ($this->items as $key => $value) {
				...
				if ($first) {
					...
					$index = $min > $val ? $key : $index;
					$min = $min > $val ? $val : $min;
				}
			}

			return $field($this->items[$index]);
		}

		public function max($field)
		{
			if (!$this->useAsCallable($field)) {
				return $this->max($this->valueRetriever($field));
			}
		...
		*/
		...
		{
			return (double) $this->sum($field) / count($this->items);
		}
