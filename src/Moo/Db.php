<?php
    ...
    class Db
    {
        ...
        public function model($data = [])
        {
            ...
            $modelFile = APPLICATION_PATH . DS . 'models' . DS . 'Raw' . DS . 'models' . DS . Inflector::lower($db) . DS . ucfirst(Inflector::lower($table)) . '.php';

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Raw')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Raw');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Raw' . DS . 'models')) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Raw' . DS . 'models');
            }

            if (!is_dir(APPLICATION_PATH . DS . 'models' . DS . 'Raw' . DS . 'models' . DS . Inflector::lower($db))) {
                File::mkdir(APPLICATION_PATH . DS . 'models' . DS . 'Raw' . DS . 'models' . DS . Inflector::lower($db));
            }

            if (!File::exists($modelFile)) {
                File::put($modelFile, str_replace('##class##', ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'RawModel', File::read(__DIR__ . DS . 'dbModel.tpl')));
            }

            $class = '\\Thin\\' . ucfirst(Inflector::lower($db)) . ucfirst(Inflector::lower($table)) . 'RawModel';
        ...
        {
            return $this->selects('created_at,updated_at');
        }
