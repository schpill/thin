<?php
    ...
    class Nodedb
    {
        ...
        public function __construct($ns, $entity)
        {
            $this->db       = 'dbNode_' . $ns . '::' . $entity;
            $entity         = "$ns::$entity";
            $this->entity   = $entity;
            $this->lock     = Inflector::camelize('node_db');
        }
        ...
        private function add($data)
        {
            ...
            if (!strlen($exists)) {
                ...
                $this->lastInsertId = $id = $this->driver()->incr($this->entity . '_nodeCount');
                $data['id'] = $id;
                ...
                if (count($data)) {
                    foreach ($indexes as $index) {
                        ...
                        if (!empty($v)) {
                            ...
                            $pattern = "index::$this->db::" . sha1(Inflector::lower($index));
        ...
        public function delete($id)
        {
            ...
            if (count($fulltextes)) {
                foreach ($fulltextes as $fulltext) {
                    $pattern = "fulltext::$this->db::" . sha1(Inflector::lower($fulltext)) . "::$id";
        ...
        {
            list($ns, $entity) = explode('::', repl('dbNode_', '', $this->db), 2);
            return Driver::instance($ns, $entity);
        }

        private function indexes()
        {
            return isAke(isAke(self::$configs, $this->db), 'indexs');
        }

        private function fulltextes()
        {
            return isAke(isAke(self::$configs, $this->db), 'fulltexts');
        }
        ...
        private static function structure($ns, $table, $fields)
        {
            ...
            $t = with(container()->tbm('core_table'))->where('name = ' . $table)->where('ns = ' . $ns)->first(true);
            if (is_null($t)) {
                $t = with(container()->tbm('core_table'))->create()->setName($table)->setNs($ns)->save();
            }
            if (!is_null($t)) {
                if (count($fields)) {
                    foreach ($fields as $field) {
                        if ('id' != $field) {
                            $f = with(container()->tbm('core_field'))->where('name = ' . $field)->first(true);
                            if (is_null($f)) {
                                $f = with(container()->tbm('core_field'))->create()->setName($field)->save();
                            }
                            $s = $dbs
                            ->where('table = ' . $t->getId())
                            ->where('field = ' . $f->getId())
        ...
        {
            $dbt = container()->tbm('core_table');
            $rows = glob(STORAGE_PATH . DS . 'dbNode_*');
            $tables = array();
            if (count($rows)) {
                foreach ($rows as $row) {
                    $db = $index = repl(array(STORAGE_PATH . DS, '.data', 'dbNode_'), '', $row);
                    list($ns, $index) = explode('::', $index, 2);
                    if (!strstr($index, 'core_')) {
                        $dbTemp = container()->tbm($index, $ns);
                        $t = $dbt->where('name = ' . $index)->where('ns = ' . $ns)->first(true);
                        if (is_null($t)) {
                            $tables[$index]['count']    = $dbTemp->countAll();
                            $data                       = $dbTemp->fetch()->exec();
                            if (count($data)) {
        ...
        {
            list($ns, $entity) = explode('::', repl('dbNode_', '', $this->db), 2);
            $t = container()->tbm('core_table')->where('name = ' . $entity)->where('ns = ' . $ns)->first(true);
            if (!is_null($t)) {
                $sts = container()->tbm('core_structure')->where('table = ' . $t->getId())->exec(true);
                if (count($sts)) {
                    foreach ($sts as $st) {
                        $st->delete();
                    }
        ...
        {
            $cached = false === $force
            ? $this->cached('RDB_allDb_' . $this->db)
            : array();
            if (empty($cached)) {
                ...
                $this->cached('RDB_allDb_' . $this->db, $collection);
                return $collection;
            } else {
                return $cached;
            }
        ...
        public function groupBy($field, $results = array())
        {
            ...
            $keyCache = sha1('RDB_groupby_' . $field . serialize($res) . $this->db);
