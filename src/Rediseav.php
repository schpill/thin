<?php
    ...
    class Rediseav
    {
        ...
        {
            $this->entity       = $entity;
            $this->dbEntity     = new Redisdb("thin_eav_entity");
            $this->dbAttribute  = new Redisdb("thin_eav_attribute");
            $this->dbValue      = new Redisdb("thin_eav_value");
            $this->dbItem       = new Redisdb("eav_" . $entity);
        }
        ...
        {
            $keyCache = sha1('existsRDB_' . serialize($name) . $this->entity);
            $cached = $this->cached($keyCache);
            if (empty($cached)) {
                $res = $this->dbEntity->findOneByName($name);
                $val = !count($res) ? false : $res->getId();
        ...
        {
            $keyCache = sha1('attributeRDB_' . serialize($a));
            $cached = $this->cached($keyCache);
            if (empty($cached)) {
        ...
        {
            $keyCache = sha1('valueRDB_' . serialize($v));
            $cached = $this->cached($keyCache);
            if (empty($cached)) {
        ...
        private function makeTab($rows, $id = null)
        {
            if (!empty($rows)) {
                $keyCache = sha1('makeTabRDB_' . serialize($rows) . $this->entity);
                $cached = $this->cached($keyCache);
                if (empty($cached)) {
                    $tab = array();
                    $first = true;
        ...
        {
            $cached = false === $force
            ? $this->cached('eavRDB_allDb_' . $this->entity)
            : array();
            if (empty($cached)) {
                $collection = array();
                ...
                foreach ($entities as $entity) {
                    ...
                    array_push($collection, $this->makeTab($items, $entity['id']));
                }
                $this->cached('eavRDB_allDb_' . $this->entity, $collection);
                return $collection;
            } else {
                return $cached;
            }
        ...
        public function groupBy($field, $results = array())
        {
            ...
            $keyCache = sha1('eavRDB_groupby_' . $field . serialize($res) . $this->entity);
