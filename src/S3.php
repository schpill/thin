<?php
    ...
    class S3
    {
        ...
        public function getBucketContents($name, $prefix = null, $marker = null, $delimeter = null, $max_keys = null)
        {
            ...
            while((string) {
                $xml->IsTruncated == 'true' && is_null($max_keys));
            }
        ...
        public function copyObject($bucket_name, $s3_path, $dest_bucket_name, $dest_s3_path)
        {
            $request = array(
                'verb' => 'PUT',
                'resource' => "/$dest_bucket_name/$dest_s3_path"
            );
            $headers = array(
        ...
        public function getAuthenticatedURL($bucket_name, $s3_path, $expires_on)
        {
            ...
            $url = sprintf(
                "http://%s.s3.amazonaws.com/%s?AWSAccessKeyId=%s&Expires=%s&Signature=%s",
                $bucket_name,
                $s3_path,
                $this->key,
                $expires_on,
        ...
        private $mimeTypes = array(
            ...
            "stm" => "text/html",
            "svg" => "image/svg+xml", "sv4cpio" => "application/x-sv4cpio",
