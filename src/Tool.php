<?php
	...
	class Tool implements ArrayAccess
	{
		/**
		...
		public function share(Closure $closure)
		{
			return function($container) use ($closure) {
				...
				static $object;
				if (is_null($object)) {
					$object = call_user_func_array($closure, array($container));
				}
				return $object;
			};
		}
		...
		public function extend($abstract, Closure $closure)
		{
			...
			if (isset($this->instances[$abstract])) {
				$args = array();
				array_push($args, $this->instances[$abstract]);
				array_push($args, $this);
				$this->instances[$abstract] = call_user_func_array($closure, $args);
				$this->rebound($abstract);
			} else {
